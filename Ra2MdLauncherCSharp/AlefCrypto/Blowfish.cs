...
/// </summary>
static file class BlowfishTable {
    /// <summary>
...
public class BlowfishContext {
    ...
    public BlowfishContext(byte[] key) {
        this._p = BlowfishTable.P.Clone() as uint[];
        this._s = BlowfishTable.S.Clone() as uint[,];
        ...
        for (i = 0; i < 18; i += 2) {
            this.Encrypt(ref datal, ref datar);
            this._p[i] = datal;
            this._p[i + 1] = datar;
        }
    ...
    private uint Loword(uint number) {
        ushort d = (ushort)(number & 0x00FF);
        number >>= 8;
        ushort c = (ushort)(number & 0x00FF);
        number >>= 8;
        ushort b = (ushort)(number & 0x00FF);
        number >>= 8;
        ushort a = (ushort)(number & 0x00FF);

        uint y = this._s[0, a] + this._s[1, b];
        y ^= this._s[2, c];
        y += this._s[3, d];
